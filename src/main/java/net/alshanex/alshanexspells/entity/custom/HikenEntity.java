        ...
        if (!this.level().isClientSide) {
            ...
            if (hitResult.getType() == HitResult.Type.BLOCK) {
                if (ServerConfigs.SPELL_GREIFING.get()) {
                    Explosion explosion = new Explosion(level(), null, ExampleSpellRegistry.HIKEN.get().getDamageSource(this, getOwner()), null, this.getX(), this.getY(), this.getZ(), this.getExplosionRadius() / 2, true, Explosion.BlockInteraction.DESTROY);
                ...
                for (LivingEntity targetEntity : entitiesInRange) {
                    if (targetEntity != this.getOwner() && targetEntity.isAlive()) {
                        ...
                        targetEntity.setSecondsOnFire(10);
                    }
                }
